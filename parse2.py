...
def parse():
    ...
    print(f"Mapped {len(addr_to_block)} block addresses.")
    ...
    for line in lines:
        ...
        if m and current_val is not None:
            target_addr = m.group(2)
            blocks[current_block_id]['edges'].append((current_val, target_addr))
            # Notice! A jmp might mean the true branch is a jne ...
            # Actually:
            # cmpl $0x100, -0x4(%rbp)
            # jne ...
            # jmp 6c68
            # This means if it IS 0x100, it jumps to 6c68!
            # So val=0x100 goes to 6c68.
            current_val = None
            continue
        ...
        if m and current_val is not None:
            target_addr = m.group(2)
            blocks[current_block_id]['edges'].append((current_val, target_addr))
            # Also, after je, there might be another cmpl, etc.
            current_val = None
            continue
    ...
    for block_id, data in blocks.items():
        ...
        for val, target_addr in data['edges']:
            # Let's find the closest block start address before target_addr
            target_block_id = None

            # Since target_addr is the exact start of the block or a nop before it:
            closest = -1
    ...
    # We want to find path to block 0x270
    target_id = 0x270
