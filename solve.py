import hashlib
import itertools

sbox = [210, 76, 176, 35, 87, 148, 91, 184, 175, 70, 158, 187, 61, 33, 103, 68, 225, 94, 216, 151, 136, 80, 52, 82, 49, 62, 44, 240, 243, 168, 118, 181, 21, 217, 211, 153, 177, 25, 190, 245, 51, 137, 113, 122, 141, 74, 27, 199, 18, 55, 37, 60, 197, 128, 248, 123, 31, 124, 131, 180, 155, 20, 3, 242, 81, 163, 247, 12, 221, 121, 93, 191, 110, 34, 147, 195, 192, 145, 159, 4, 86, 220, 98, 100, 154, 6, 235, 206, 79, 135, 222, 169, 189, 218, 170, 146, 54, 47, 227, 10, 64, 249, 13, 39, 236, 106, 102, 24, 156, 230, 28, 32, 207, 244, 186, 99, 40, 160, 174, 254, 204, 8, 228, 41, 182, 50, 120, 30, 167, 15, 140, 178, 212, 253, 63, 107, 150, 193, 171, 36, 78, 119, 9, 14, 23, 111, 251, 139, 117, 19, 138, 56, 172, 83, 85, 114, 116, 105, 48, 233, 166, 58, 46, 133, 194, 65, 11, 89, 185, 183, 16, 38, 229, 203, 255, 26, 132, 165, 57, 162, 84, 7, 129, 2, 1, 42, 246, 196, 152, 143, 250, 142, 101, 59, 201, 90, 241, 95, 130, 66, 29, 214, 69, 77, 198, 67, 43, 17, 5, 252, 215, 232, 179, 73, 75, 213, 200, 239, 238, 188, 223, 108, 96, 202, 226, 237, 97, 231, 161, 71, 219, 209, 53, 45, 109, 208, 149, 22, 134, 115, 88, 72, 224, 127, 157, 205, 126, 104, 92, 0, 144, 164, 112, 173, 125, 234]
sbox_inv = [None]*256
for i in range(256):
    sbox_inv[sbox[i]] = i

c_hex = "f841cff08a6c0640848efc500a1c6a6cf206546e55542912b2f3540ae043933cda69dd45d778ea9701410416413b67f9f57e9feff06caabb3811149b020155a4d469dd93c10bd2c9cc34be7881a66a243a11acc9b10bf1496d206ca9d5d92ce4f6a853d2bba7db40584196503b015bc3"
C = bytes.fromhex(c_hex)
P = b"I have a secret, please don't share: "

chunks_c = [C[i:i+16] for i in range(0, len(C), 16)]
chunks_p = [P[i:i+16] for i in range(0, len(P), 16)]

k0_candidates = [[] for _ in range(16)]

for i in range(16):
    for k0_i in range(256):
        # We need C_0_i ^ C_1_i == S(P_0_i ^ k0_i) ^ S(P_1_i ^ k0_i)
        p0 = chunks_p[0][i]
        c0 = chunks_c[0][i]
        p1 = chunks_p[1][i]
        c1 = chunks_c[1][i]

        val1 = sbox[p0 ^ k0_i]
        val2 = sbox[p1 ^ k0_i]

        if c0 ^ c1 == val1 ^ val2:
            # If we also have a third block, we can verify
            valid = True
            if len(chunks_p) > 2 and i < len(chunks_p[2]):
                p2 = chunks_p[2][i]
                c2 = chunks_c[2][i]
                val3 = sbox[p2 ^ k0_i]
                if c0 ^ c2 != val1 ^ val3:
                    valid = False

            if valid:
                k0_candidates[i].append(k0_i)

for k0 in itertools.product(*k0_candidates):
    k0 = bytes(k0)
    k1 = hashlib.md5(k0).digest()

    # check if this k0 and k1 matches the known plaintext chunks
    c0_computed = [sbox[p ^ k] ^ k1_i for p, k, k1_i in zip(chunks_p[0], k0, k1)]
    if bytes(c0_computed) == chunks_c[0]:
        print("Found key!")
        print(f"k0 = {k0.hex()}")
        print(f"k1 = {k1.hex()}")

        # decrypt all chunks
        decrypted = b""
        for chunk in chunks_c:
            p = [sbox_inv[c ^ k1_i] ^ k0_i for c, k0_i, k1_i in zip(chunk, k0, k1)]
            decrypted += bytes(p)
        print(decrypted)
        break
